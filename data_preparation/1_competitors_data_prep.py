...
output_file = 'competitors_modified.csv'
...
# Define columns to keep (excluding promo columns, maybe even sales and revenue?)
columns_to_keep = ['product_id', 'store_id',
                   'date', 'sales', 'revenue', 'stock', 'price']
...
def generate_random_date():
    ...
    random_timestamp = start_date + \
        pd.Timedelta(days=np.random.randint(
            0, (end_date - start_date).days + 1))
    return random_timestamp.strftime('%Y-%m-%d')


reader = pd.read_csv(input_file, usecols=columns_to_keep, chunksize=chunk_size)
first_chunk = True
...
for chunk in reader:
    ...
    # Generate random dates for 2025
    chunk['date'] = [generate_random_date() for _ in range(len(chunk))]
